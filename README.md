...
由于新注册账号没有绑定过手环，会导致华米拦截向微信同步步数。

> **本仓库已内置「账号识别 + 自动绑定虚拟设备」能力**：登录后若检测到账号未绑定任何小米手环/手表设备，会自动调用第三方接口为该账号绑定一台虚拟设备，随后再上传步数，无需手动去网站绑定。该能力默认开启，已内置 API Key，开箱即用。如需更换接口密钥，可在环境变量 / CONFIG 中配置 `BIND_API_KEY`，自定义设备名可配置 `BIND_DEVICE_NAME`（6 字以内）。

如需手动绑定或自测，仍可参考以下第三方站点：
